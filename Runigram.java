...
public class Runigram {
	...
	public static Color[][] read(String fileName) {
		...
		In inputStream = new In(fileName); // Renamed variable to avoid conflict
    String format = inputStream.readString(); // Now use inputStream to read
    if (!format.equals("P3")) {
	...
	if (numRows <= 0 || numCols <= 0) {
        throw new IllegalArgumentException("Invalid image dimensions: " + numRows + " x " + numCols);
    }
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		...
		for (int i = 0; i < numRows; i++) {
			for (int j = 0; j < numCols; j++) {
				flipped[i][numCols - j - 1] = image[i][j];
			}
		}
		return flipped;
	}
	...
	public static Color[][] flippedVertically(Color[][] image){
		...
		for (int i = 0; i < numRows; i++) {
			for (int j = 0; j < numCols; j++) {
				flipped[numRows - i - 1][j] = image[i][j];
			}
		}
		return flipped;
	}
